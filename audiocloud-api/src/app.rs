...
impl SessionPacket {
    pub fn push_fixed_instance_metering(&mut self, instance: FixedId, reports: HashMap<ReportId, MultiChannelValue>) {
        ...
        for (report_id, value) in reports {
            fixed
                ...
                .entry(report_id)
                .or_default()
                .push(DiffStamped::from((self.created_at, value)));
        }
    }
    ...
    pub fn push_dynamic_output_metering(&mut self, dynamic_id: &DynamicId, output: MultiChannelValue) {
        ...
        dynamic
            .output_metering
            .push(DiffStamped::from((self.created_at, output)));
    }
