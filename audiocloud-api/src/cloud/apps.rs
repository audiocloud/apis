...
use crate::session::{MixerChannels, SessionConnection, SessionDynamicInstance, SessionFixedInstance, SessionFlowId, SessionMixer, SessionSecurity, SessionTrack};
...
impl SessionSpec {
    ...
    fn validate_connection(&self, id: &ConnectionId, connection: &SessionConnection, models: &HashMap<ModelId, Model>) -> Result<(), CloudError> {
    ...
    fn check_channel_exists(&self, id: &ConnectionId, flow_id: &SessionFlowId, channels: &MixerChannels, models: &HashMap<ModelId, Model>) -> Result<(), CloudError> {
        match flow_id {
            SessionFlowId::MixerInput(mixer_id) => {
                self.check_channel_exists_mixer(id, mixer_id, channels)
            }
            SessionFlowId::MixerOutput(mixer_id) => {
                self.check_channel_exists_mixer(id, mixer_id, channels)
            }
            SessionFlowId::FixedInstanceInput(fixed_id) => {
                self.check_channel_exists_fixed(id, fixed_id, channels, false, models)
            }
            SessionFlowId::FixedInstanceOutput(fixed_id) => {
                self.check_channel_exists_fixed(id, fixed_id, channels, true, models)
            }
            SessionFlowId::DynamicInstanceInput(dynamic_id) => {
                self.check_channel_exists_dynamic(id, dynamic_id, channels, false, models)
            }
            SessionFlowId::DynamicInstanceOutput(dynamic_id) => {
                self.check_channel_exists_dynamic(id, dynamic_id, channels, true, models)
            }
            SessionFlowId::TrackOutput(track_id) => {
                self.check_channel_exists_track(id, track_id, channels)
            }
        }
    }

    fn check_channel_exists_mixer(&self, id: &ConnectionId, mixer_id: &MixerId, channels: &MixerChannels) -> Result<(), CloudError> {
        let mixer = self.mixers.get(mixer_id).ok_or_else(|| {
            InternalInconsistency(format!("Connection {id} flow to mixer {mixer_id} does not exist"))
        })?;
    ...
    fn check_channel_exists_fixed(&self, id: &ConnectionId, fixed_id: &FixedId, channels: &MixerChannels, output: bool, models: &HashMap<ModelId, Model>) -> Result<(), CloudError> {
        let fixed = self.fixed.get(fixed_id).ok_or_else(|| {
            InternalInconsistency(format!("Connection {id} references fixed {fixed_id} which does not exist"))
        })?;
    ...
    fn check_channel_exists_dynamic(&self, id: &ConnectionId, dynamic_id: &DynamicId, channels: &MixerChannels, output: bool, models: &HashMap<ModelId, Model>) -> Result<(), CloudError> {
    ...
    fn check_channel_exists_track(&self, id: &ConnectionId, track_id: &TrackId, channels: &MixerChannels) -> Result<(), CloudError> {
        let track = self.tracks.get(track_id).ok_or_else(|| {
            InternalInconsistency(format!("Connection {id} references track {track_id} which does not exist"))
        })?;
