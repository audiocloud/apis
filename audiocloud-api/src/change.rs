...
use crate::session::{
  MixerInput, MixerInputValues, Session, SessionDynamicInstance, SessionFixedInstance, SessionMixer, SessionMixerId, SessionObjectId, SessionTimeSegment,
  SessionTrack, SessionTrackChannels, SessionTrackMedia,
};
...
impl Session {
  ...
  pub fn get_media_object_ids<'a>(&'a self) -> impl Iterator<Item = &'a MediaObjectId> + 'a {
    self.tracks.values().flat_map(|track| track.media.values().map(|media| &media.object_id))
  }
  ...
  pub fn apply_change(&mut self, modify: ModifySession) -> Result<(), ModifySessionError> {
    match modify {
      | ModifySession::AddFixedInstance { fixed_id: mixer_id,
                                          process, } => self.add_fixed_instance(mixer_id, process),
      | ModifySession::AddDynamicInstance { dynamic_id: mixer_id,
                                            process, } => self.add_dynamic_instance(mixer_id, process),
      | ModifySession::AddMixer { mixer_id, mixer: channels } => self.add_mixer(mixer_id, channels),
      | ModifySession::DeleteMixer { mixer_id } => self.delete_mixer(mixer_id),
      | ModifySession::DeleteMixerInput { mixer_id, input_id } => self.delete_mixer_input(mixer_id, input_id),
      | ModifySession::AddMixerInput { mixer_id, input_id, input } => self.add_mixer_input(mixer_id, input_id, input),
      | ModifySession::SetFixedInstanceParameterValues { fixed_id: id, values } => self.set_fixed_instance_parameter_values(id, values),
      | ModifySession::SetDynamicInstanceParameterValues { dynamic_id: id, values } => self.set_dynamic_instance_parameter_values(id, values),
      | ModifySession::DeleteInputsReferencing { source_id } => self.delete_inputs_referencing(source_id),
      | ModifySession::AddTrack { track_id, channels } => self.add_track(track_id, channels),
      | ModifySession::DeleteTrackMedia { track_id, media_id } => self.delete_track_media(track_id, media_id),
      | ModifySession::DeleteTrack { track_id } => self.delete_track(track_id),
      | ModifySession::SetInputValues { mixer_id,
                                        input_id,
                                        values, } => self.set_input_values(mixer_id, input_id, values),
      | ModifySession::AddTrackMedia { track_id,
                                       media_id,
                                       channels,
                                       media_segment,
                                       timeline_segment,
                                       object_id, } => self.add_track_media(track_id, media_id, channels, media_segment, timeline_segment, object_id),
      | ModifySession::SetTrackMediaValues { track_id,
                                             media_id,
                                             channels,
                                             media_segment,
                                             timeline_segment,
      ...
      | ModifySession::SetSecurity { key, security } => self.set_security(key, security),
      | ModifySession::RevokeSecurity { key } => self.revoke_security(key),
    }
  }
  ...
  fn delete_input(inputs: &mut HashMap<InputId, MixerInput>, mixer_id: &SessionMixerId, input_id: InputId) -> Result<(), ModifySessionError> {
  ...
  fn add_input(output: &mut HashMap<InputId, MixerInput>, mixer_id: &SessionMixerId, input_id: &InputId, input: MixerInput) -> Result<(), ModifySessionError> {
  ...
  pub fn set_input_values(&mut self, mixer_id: SessionMixerId, input_id: InputId, values: MixerInputValues) -> Result<(), ModifySessionError> {
    let inputs = match &mixer_id {
      | SessionMixerId::Mixer(m) => self.mixers.get_mut(&m).map(|m| &mut m.inputs).ok_or(MixerDoesNotExist(m.clone()))?,
      | SessionMixerId::FixedInstance(m) => self.fixed.get_mut(&m).map(|m| &mut m.inputs).ok_or(FixedInstanceDoesNotExist(m.clone()))?,
      | SessionMixerId::DynamicInstance(m) => self.dynamic.get_mut(&m).map(|m| &mut m.inputs).ok_or(DynamicInstanceDoesNotExist(m.clone()))?,
    };
  ...
  pub fn add_track(&mut self, track_id: TrackId, channels: SessionTrackChannels) -> Result<(), ModifySessionError> {
    ...
    self.tracks.insert(track_id, SessionTrack { channels,
  ...
  pub fn add_track_media(&mut self,
    ...
    track.media.insert(media_id, SessionTrackMedia { channels,
                                                     media_segment,
                                                     timeline_segment,
                                                     object_id });
  ...
  pub fn set_track_media_values(&mut self,
                                ...
                                -> Result<(), ModifySessionError> {
    let track = self.tracks.get_mut(&track_id).ok_or(TrackDoesNotExist(track_id.clone()))?;
    let media = track.media.get_mut(&media_id).ok_or(MediaDoesNotExist(track_id.clone(), media_id))?;
...
fn security_changes(rv: &mut Vec<ModifySession>, existing: &HashMap<SecureKey, SessionSecurity>, new: &HashMap<SecureKey, SessionSecurity>) {
